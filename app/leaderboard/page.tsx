...
const MAX_TIME_BONUS = 60; // maksimum hız bonusu
...
const calcScore = (activeStems: string[], elapsedSec: number): number => {
  ...
  const timeBonus = Math.max(0, MAX_TIME_BONUS - Math.floor(elapsedSec));
  return Math.max(20, 100 - stagePenalty) + timeBonus;
};
...
export default function Home() {
  ...
  const playStemsFrom = (activeList: string[], offset = 0) => {
    ...
    STEM_ORDER.forEach(stem => {
      ...
      src.buffer = buffer;
      src.loop = true;
      gain.gain.value = activeList.includes(stem) ? 1 : 0;
      src.connect(gain);
  ...
  const handleGuess = (e: React.FormEvent) => {
    ...
    if (normalize(guessInput) === normalize(currentSong?.title || '')) {
      ...
      // Yanlış: input shake + kırmızı border, ses devam eder
      setShakeInput(true);
      setWrongBorder(true);
